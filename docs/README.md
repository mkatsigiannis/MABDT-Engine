# Documentation

- [Architecture](architecture.md) — what `mabdt` is and how `tiger_motors_dt`
  builds on it, mapped to JIM §3.
- [`../examples/toy_line/README.md`](../examples/toy_line/README.md) — the
  smallest working deployment.

The source carries the rest. Module docstrings explain intent; class
docstrings explain shape. Start at [`../mabdt/__init__.py`](../mabdt/__init__.py)
for the engine's public API surface.
